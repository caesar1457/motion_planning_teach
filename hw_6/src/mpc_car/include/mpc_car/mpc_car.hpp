...
class MpcCar {
 private:
  ...
  void linearization(const double& phi,
    ...
    auto tan_delta = tan(delta);
    auto cos_delta = cos(delta);
    Ad_(0, 2) = -v * sin_phi * dt_;
    Ad_(0, 3) = cos_phi * dt_;
    Ad_(1, 2) = v * cos_phi * dt_;
    Ad_(1, 3) = sin_phi * dt_;
    Ad_(2, 3) = tan_delta / ll_ * dt_;
    Bd_(2, 1) = v / (ll_ * cos_delta * cos_delta) * dt_;
    Bd_(3, 0) = 1 * dt_;
    gd_(0) = v*phi*sin_phi * dt_;
    gd_(1) = -v*phi*cos_phi * dt_;
    gd_(2) = -v*delta / (ll_ * cos_delta * cos_delta) * dt_;
    return;
  }
 ...
 public:
  MpcCar(rclcpp::Node::SharedPtr node) : node_(node) {
    ...
    for (int i = 0; i < N_; ++i) {
      ...
      if (i < N_ - 1) {
        Cu_.coeffRef(i * 3 + 2, i * m + 1) = - 1.0 / dt_;
        Cu_.coeffRef(i * 3 + 2, (i + 1) * m + 1) = 1.0 / dt_;
  ...
  int solveQP(const VectorX& x0_observe) {
    ...
    for (int i = 0; i < N_; ++i) {
      ...
      if (i == 0) {
        ...
        BB.block(i * n, 0, n, m * i) = Ad_ * BB.block((i - 1) * n, 0, n, m * i);
      ...
      if ( i == N_ -1) {
        qx.coeffRef(i * n + 0, 0) = -1.0 * xy(0) * rhoN_;
        qx.coeffRef(i * n + 1, 0) = -1.0 * xy(1) * rhoN_;
        qx.coeffRef(i * n + 2, 0) = -1.0 * rho_ * phi * rhoN_;
      } else {
        qx.coeffRef(i * n + 0, 0) = -1.0 * xy(0);
