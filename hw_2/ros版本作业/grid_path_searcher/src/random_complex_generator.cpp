...
{  
   ...
   for(int i = 0; i < _cir_num; i ++)
   {
      ...
      for(auto pt: circle_set)
      {
         ...
         // alignment
         pt_random.x = floor(pt_random.x/_resolution) * _resolution + _resolution / 2.0;
         pt_random.y = floor(pt_random.y/_resolution) * _resolution + _resolution / 2.0;
         pt_random.z = floor(pt_random.z/_resolution) * _resolution + _resolution / 2.0;
         if(pt_random.z >= 0.0)
      ...
      for(int r = -widNum/2.0; r < widNum/2.0; r ++ )
      {
         for(int s = -widNum/2.0; s < widNum/2.0; s ++ )
         {
            ...
            for(int t = 0; t < heiNum; t ++ ){
               ...
               pt_random.z =     (t+0.0) * _resolution + 0.001;
               cloudMap.points.push_back( pt_random );
            }
         }
      }
